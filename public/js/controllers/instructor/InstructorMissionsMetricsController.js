angular
  ...
  .controller('InstructorMissionsMetricsController', [
    ...
    '$location',
    '$mdDialog',
    function($scope, $window, $http, $routeParams, $location, $mdDialog) {
      ...
      $scope.addRosterClicked = function(e) {
        console.log('clicked!', $scope.model);
      ...
      function InstructorCreateEditClassController(
        $scope,
        $mdDialog,
        classInfo
      ) {
      ...
      function InstructorCopyClassController($scope, $mdDialog, allClasses) {
        ...
        $scope.submit = function() {
          $http({
            method: 'POST',
            url:
              '/stats/instructors/' + $routeParams.sid + '/classes/copyClass',
            data: {
              name: $scope.class.name,
              description: $scope.class.description,
      ...
      function aggregateCT(raw_data) {
        for (var i = 0; i < raw_data.length; i++) {
          for (var j = 0; j < raw_data[i].enrolled.length; j++) {
            raw_data[i].enrolled[j].missionCT = missionCT_cal(
              raw_data[i].enrolled[j].missionCT
            );
          }
        }
        return raw_data;
      ...
      $http.get('missionmetrics/' + sid).then(function(response) {
        if (response.status == '403') {
          $window.location.href = '/public/views/error.html';
        } else {
          $scope.raw_data_missions = response.data;

          var data_missions = aggregateCT($scope.raw_data_missions);
          var data_missions_chart = data2chart_m(data_missions);
          $scope.chart = prepareChart_Ins(data_missions_chart);
        }
      });

      function data2chart_m(data_missions) {
        var title = 'Missions CT score';
        var series_data = [];
        for (var i = 0; i < data_missions.length; i++) {
          series_data[i] = {};
          series_data[i].name = data_missions[i].missionName;
          series_data[i].data = [0, 0, 0, 0, 0, 0, 0];
          for (var j = 0; j < data_missions[i].enrolled.length; j++) {
            for (var k = 0; k < series_data[i].data.length; k++) {
              series_data[i].data[k] +=
                data_missions[i].enrolled[j].missionCT[k];
            }
          }
          for (var k = 0; k < series_data[i].data.length; k++) {
            series_data[i].data[k] /= data_missions[i].enrolled.length;
          }
        }
        var res = {
          title: title,
